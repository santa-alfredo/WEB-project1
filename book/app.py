...
def book_form(book_id):
    ...
    sql=text("select b.*, count(id_book), case when count(rating)=0 then 0 else round(avg(rating)) end from books as b full outer join user_book on b.id=id_book where b.id=:id group by b.id").params(id=book_id)
